...
Two important points to remark are the following: (a) the expressive power of a geometric language is greatly empowered by  currying, i.e.~by the technique of translating the evaluation of a function that takes multiple arguments (or a tuple of arguments) into evaluating a sequence of functions, each with a single argument; (b) Python/pyplasm are currently used even to teach (geometric) programming to K12 students~\cite{}. Most of plugin templates currently used by Metior were developed in class by the students of the graphics course taught by one of the authors. 
