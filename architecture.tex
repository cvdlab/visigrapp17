...
\vspace{-3mm}\subsection{UI and user experience}\vspace{-3mm}

The web application presents itself as a simplified CAD, where the user interface comprises three main areas of interaction: \emph{toolbar}, \emph{canvas} and \emph{sidebar}, as shown by Figures~\ref{fig:ui2}a and~\ref{fig:ui2}b.
...
In Figure~\ref{spiralstair} it is shown the plugin used to generate a concrete spiral stair, where the steps are obtained by coded update of the polyhedral approximation of a \texttt{larSolidHelicoid()}, with parametric minor and major radiuses (\texttt{r} and \texttt{R}) and int number of \texttt{steps}. The \texttt{nturns} float is in radians.
...
\vspace{2mm}
   \includegraphics[width=\linewidth]{images/frame}
      \caption{A spatial concrete frame developed by the \texttt{pyplasm} plugin \texttt{spatialFrame} from a simple \texttt{.csv} input.}
   \label{spiralstair}
\end{figure}
